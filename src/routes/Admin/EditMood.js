...
import { createMood, updateMood, getMoodbyId } from '../../backend/helpers/mood';

function EditMood({ match, ...props }) {

    // get params from url : moodId
    const { params } = match;
    ...
    const preload = () => {
        getMoodbyId(params.moodId).then((resp) => {
            if (!resp || resp.error) {
                setStatus({
                    error: true,
                    success: false,
                ...
                setValues({
                    ...values,
                    name: resp.mood.name,
                    description: resp.mood.description,
                });
            }
        })
    }
    ...
    //  Go back to all moods page
    const goBack = () => (
        <Link to="/admin/moods" className="text-white button d-block text-center button-secondary1 w-100 py-2 mt-3">
            Go Back
        </Link>
    )
