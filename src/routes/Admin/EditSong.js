...
import { getAllArtists, getArtistbyId } from '../../backend/helpers/artist';
...
import { createSong, getSongbyId, updateSong } from '../../backend/helpers/song';
import { getDurMin } from '../../components/helpers/audio';
import { BACKEND } from '../../backend/api';

function EditSong({ match, ...props }) {
    ...
    const preloadSong = () => {
        ...
        getSongbyId(params.songId).then((resp) => {
            if (!resp || resp.error) {
                setStatus({
                    error: true,
                    success: false,
                ...
                if (resp.song.file_path) {
                    file_path = `${BACKEND}/${resp.song.file_path}`
                }
                if (resp.song.thumbnail) {
                    thumbnail = `${BACKEND}/${resp.song.thumbnail}`
                }
                formData.set('id', params.songId);
                setValues({
                    ...values,
                    name: resp.song.song_name,
                    description: resp.song.song_description,
                    length: resp.song.length,
                    artist_id: resp.song.artist_id,
                    genre_id: resp.song.genre_id,
                    mood_id: resp.song.mood_id,
                    song_file: file_path,
                    thumbnail: thumbnail,
                });
            }
    ...
    const handleInputs = (name) => {
        ...
        if (name !== "song_file" && name !== "thumbnail" && values[name]) {
    ...
    const onSubmit = (e) => {
        ...
        updateSong(formData, token).then((resp) => {
            if (!resp || resp.error) {
                setStatus({
                    ...status,
                    msg: resp.error,
